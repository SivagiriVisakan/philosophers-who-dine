...
int get_left(int i)
{
    ...
    if(sem_trywait(&forks[left_fork_index]) == 0)
    {

        snprintf(philosopher_info[i].fork_held, 200, "Left fork success\n");
...
int get_right(int i)
{
    ...
    if(sem_trywait(&forks[right_fork_index]) == 0)
    {
...
void *philosopher(void *arg)
{
    ...
    {
        think(i);
        snprintf(philosopher_info[i].fork_held, 200, "Waiter busy\n");
        pthread_mutex_lock(&waiter); // Reserve the waiter
        snprintf(philosopher_info[i].fork_held, 200, "Acquired waiter, trying left fork\n");
        int left = get_left(i);
        if(left == -1){
            pthread_mutex_unlock(&waiter);
            snprintf(philosopher_info[i].fork_held, 200, "Left fork not free - continue thinking%d\n\0",left);
            continue;
        }
        ...
        if(right == -1){
            sem_post(&forks[left]);
            pthread_mutex_unlock(&waiter);
            snprintf(philosopher_info[i].fork_held, 200, "Right fork not free - continue thinking%d\n\0",left);
            continue;
        }
        pthread_mutex_unlock(&waiter);
...
int main(int argc, char const *argv[])
{
    ...
    pthread_mutex_init(&waiter, NULL); // Initialize the waiter mutex
    srand(time(NULL));
