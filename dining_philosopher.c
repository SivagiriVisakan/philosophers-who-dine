...
pthread_mutex_t chopsticks[TOTAL_PHILOSOPHERS];
pthread_t philosophers[TOTAL_PHILOSOPHERS];
pthread_attr_t attributes[TOTAL_PHILOSOPHERS];
...
void pickUp(int philosopherNumber) {
	int right = (philosopherNumber + 1) % TOTAL_PHILOSOPHERS;
	int left = (philosopherNumber + TOTAL_PHILOSOPHERS) % TOTAL_PHILOSOPHERS;
...
void putDown(int philosopherNumber) {
	...
	pthread_mutex_unlock(&chopsticks[(philosopherNumber + 1) % TOTAL_PHILOSOPHERS]);
	pthread_mutex_unlock(&chopsticks[(philosopherNumber + TOTAL_PHILOSOPHERS) % TOTAL_PHILOSOPHERS]);
}